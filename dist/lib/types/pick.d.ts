 ...
 */
export type ObjectPath<T extends Record<string, any>, N extends string = '', K = keyof T> = K extends string ? (T[K] extends Record<string, any> ? ObjectPath<T[K], N extends '' ? K : `${N}.${K}`> : N extends '' ? K : `${N}.${K}`) | (N extends '' ? '' : N) : string;
/**
 * 深度解析鏈上的型態
 * @example
 * const foo = {
 ...
 */
export type PeelType<P extends string, T extends Record<string, any>> = P extends `${infer H}.${infer S}` ? PeelType<S, T[H]> : T[P];
export {};
